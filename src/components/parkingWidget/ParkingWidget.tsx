...
function ParkingWidget() {
    ...
    const formattedLocations = {
      ...
      features: locations.features.map((feature) => ({
        ...
        properties: {
          ...feature.properties,
          total_occupancy:
            (Number(feature.properties.FreeSpaceShort) +
              Number(feature.properties.FreeSpaceLong)) /
            (Number(feature.properties.ShortCapacity) +
              Number(feature.properties.LongCapacity)),
        },
      })),
    };
  ...
  const { flyToLocation } = useSelector(
    (state: RootState) => state.parkingWidget,
  );
