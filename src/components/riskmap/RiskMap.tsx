...
const RiskMap = ({ title, filePath, opacityLevel, weight, zoom }: RiskMapProps) => {
    ...
    const [geoJSONDataHeatmap, setDataHeatmap] = useState<GeoJSON.FeatureCollection<GeoJSON.Geometry>>();

    const [selectedProcesses, setSelectedProcesses] = useState<string[]>([]);
    const [selectedRoadNames, setSelectedRoadNames] = useState<string[]>([]);
    const [selectedStartTime, setSelectedStartTime] = React.useState<Dayjs | null>(dayjs());
    const [selectedEndTime, setSelectedEndTime] = React.useState<Dayjs | null>(dayjs());
    const [filteredAccidentData, setFilteredAccidentData] = useState<Array<AccidentData>>([]);
    ...
    const handleDirectionChange = (event: React.MouseEvent<HTMLElement>, newDirections: string[]) => {
        const lastPressed = (event.currentTarget as HTMLButtonElement).value;

        if (lastPressed === 'Both') {
            setSelectedDirections((prevDirections) => (prevDirections.includes('Both') ? [] : ['Both']));
        } else {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            // Set the map data
            const { featureCollectionPoint, featureCollectionSegment, fullFeatureCollection } =
            ...
            setDataHeatmap(fullFeatureCollection as GeoJSON.FeatureCollection<GeoJSON.Geometry>);
            setLoading(false);
        };
        fetchData();
    ...
    useEffect(() => {
        ...
        const addSourcesAndLayers = () => {
            ...
            geoJSONDataHeatmapLeft.features = geoJSONDataHeatmapLeft.features.filter(
                (feature) => feature.properties?.zijde === 'L',
            );
            ...
            map.addLayer(
                heatmapLayer('accidentsHeatmapLeft', 'accidentsSourceHeatmapLeft', opacityLevel, weight) as AnyLayer,
            );

            const geoJSONDataHeatmapRight = JSON.parse(JSON.stringify(geoJSONDataHeatmap)) as GeoJSON.FeatureCollection;
            geoJSONDataHeatmapRight.features = geoJSONDataHeatmapRight.features.filter(
                (feature) => feature.properties?.zijde !== 'L',
            );
            ...
            map.addLayer(
                heatmapLayer('accidentsHeatmapRight', 'accidentsSourceHeatmapRight', opacityLevel, weight) as AnyLayer,
            );
            ...
            map.addLayer(
                heatmapLayer('accidentsHeatmapAll', 'accidentsSourceHeatmapAll', opacityLevel, weight) as AnyLayer,
            );
        };
    ...
    useEffect(() => {
        ...
        const setSelectedDirVisbility = (visible: string) => {
            selectedDirections.forEach((direction) => {
                switch (direction) {
                    case 'L':
                        setVisibility(['accidentsHeatmapLeft'], visible);
                        break;
                    case 'R':
                        setVisibility(['accidentsHeatmapRight'], visible);
                        break;
                    case 'Both':
        ...
        if (heatmapVisible) {
            setSelectedDirVisbility('visible');
        } else {
    ...
    useMemo(() => {
        ...
        if (selectedProcesses.length !== 0 || selectedRoadNames.length !== 0) {
            filteredData = accidentData.filter(
                (location) =>
                    (selectedProcesses.length === 0 ? true : selectedProcesses.includes(location.Proces)) &&
                    (selectedRoadNames.length === 0 ? true : selectedRoadNames.includes(location.Weg)),
            );
        }
        ...
        filteredData = filteredData.filter(
            (location) =>
                location.Startdatum.getTime() >= (selectedStartTime === null ? 0 : selectedStartTime.unix()) * 1000 &&
                location.Startdatum.getTime() <= (selectedEndTime === null ? 0 : selectedEndTime.unix()) * 1000,
        );
        setFilteredAccidentData(filteredData);
    }, [accidentData, selectedProcesses, selectedRoadNames, selectedStartTime, selectedEndTime]);

    const finalListWithDirection = useMemo(() => {
        if (!selectedDirections.includes('Both')) {
            return filteredAccidentData.filter((location) => {
                return selectedDirections.some((direction) => location.Zijde.startsWith(direction));
            });
        } else {
            ...
            <Paper elevation={0} sx={{ width: '40%', position: 'relative', overflow: 'auto' }}>
                ...
                <List>
                    ...
                    <Box display='flex' alignItems='flex-start' sx={{ width: '100%' }}>
                        <Box display='flex' alignItems='center' sx={{ paddingLeft: 7, width: '50%' }}>
                            <Typography variant='body2'>Show Points</Typography>
                            <Switch checked={pointsVisible} onChange={handlePointsSwitchChange} />
                        </Box>
                        <Box display='flex' alignItems='center' sx={{ width: '50%' }}>
                            <Typography variant='body2'>Show Heatmap</Typography>
                            <Switch checked={heatmapVisible} onChange={handleHeatmapSwitchChange} />
                        </Box>
                    </Box>
                    <Box display='flex' alignItems='center' sx={{ width: '100%', paddingBottom: 1 }}>
                        <Typography variant='body2' sx={{ paddingLeft: 9, paddingRight: 3 }}>
                    ...
                    <Box display='flex' alignItems='center' sx={{ width: '100%', paddingBottom: 1 }}>
                        ...
                        >
                            ...
                            {Array.from(new Set(accidentData.map((location) => location.Proces)))
                                .sort()
                                .map((proces) => (
                    ...
                    <Box display='flex' alignItems='center' sx={{ width: '100%' }}>
                        <Typography variant='body2' sx={{ paddingLeft: 10, paddingRight: 3 }}>
                            Filter by Road:
                        </Typography>
                        <Select
                            multiple
