...
export const useSelectAccident = (map: mapboxgl.Map, geoJSONDataHeatmap:  FeatureCollection<Geometry, GeoJsonProperties>) => {
    ...
    useEffect(() => {
        ...
        map!.setPaintProperty('accidentsLayerPoint', 'circle-radius', ['case', ['==', ['get', 'gid'], selectedAccidentID], 10, 6]);
        map!.setPaintProperty('accidentsLayerPoint', 'circle-stroke-color', ['case', ['==', ['get', 'gid'], selectedAccidentID], 'yellow', '#FFF']);
        
        map!.flyTo({ center: ((geoJSONDataHeatmap?.features.find(feature => feature?.properties?.gid === selectedAccidentID)?.geometry as GeoJSON.Point).coordinates) as LngLatLike , zoom: 14 });
